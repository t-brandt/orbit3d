...
def initialize_data(config, companion_gaia):
    ...
    data = orbit.Data(HipID, RVFile, AstrometryFile, relRVfile=relRVFile, companion_gaia=companion_gaia)
...
def lnprob(theta, returninfo=False, RVoffsets=False, use_epoch_astrometry=False,
    ...
    if priors is not None:
        ...
        return lnp + orbit.calcL(data, params, model) - 0.5*(params.mpri - priors['mpri'])**2/priors['mpri_sig']**2
    else:
